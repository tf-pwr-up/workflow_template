...
5. **Check if E2E tests verify user journeys** — for user-facing features, verify there's at least one E2E test that completes a full workflow (navigate → interact → verify result). A test that just checks 'page loads' is not sufficient — mark as WARN. Specifically:
   - Every primary entity type must have a lifecycle test (create → view detail → edit → verify → delete)
   - Every form that submits data must have a test that fills it, submits it, and verifies the data persisted
   - Every page that fetches and displays data must be visited with real data present in at least one test
   - Every multi-step workflow must be tested end-to-end
