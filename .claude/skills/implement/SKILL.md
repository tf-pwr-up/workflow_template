...
- Follow the `/e2e` skill for conventions — especially the no-mocking rule and coverage requirements
...
- Test that created items appear in lists, can be viewed on detail pages, can be edited, and can be deleted
- A test that just goes to a URL and checks the page container is visible is a smoke test, not an E2E test
- **Every data-fetching page must be visited with real data** — pages that load data from the API are high-risk for integration bugs (response shape mismatches, infinite loops, error handling) and MUST be exercised in E2E tests

**Per-batch E2E minimum (incremental builds):**
For each batch with user-facing changes, analyze what was added/changed and write E2E tests that exercise it:
- Every new form must have a test that fills it, submits it, and verifies the data persisted
- Every new page that displays data must be visited with real data present
- Every new workflow must be tested end-to-end
- Changed features must have their E2E tests updated to verify the new behaviour (not just updated selectors)
...
6. **Test file count verification (BLOCKING)**:
   ...
   - **Test quality verification**: For each test file, verify it contains at least one test that exercises actual behaviour (not just `toBeDefined()`, `typeof`, or import checks). Tests that only verify existence are equivalent to no tests — rewrite them.
   - **Coverage ratio enforcement**: If test files < 80% of production files for the current batch, STOP and write the missing tests. Every page, hook, store, service, and utility file needs a test.
