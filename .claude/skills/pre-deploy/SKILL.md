...
3. **FAIL conditions** (any one = NOT READY):
   - Fewer than 8 journey tests total
   - Journey test count < smoke test count (the suite is mostly smoke)
   - No golden path test (create → view → edit → verify → delete cycle)
   - No form submission test that verifies data persists
   - No auth journey test that completes a login flow
   - No edit flow test (load existing entity → modify → save → verify changes)
   - No error handling test (form validation error, unauthorized access)
   - No logout test that verifies session is cleared
...
The E2E suite must cover not just happy paths but also error handling, edge cases, and the full CRUD lifecycle. This agent verifies breadth and depth of E2E coverage.

1. **Feature-to-test mapping**: For every user-facing feature in the app (pages, forms, workflows), identify which E2E test exercises it. Features with no E2E coverage = FAIL.
2. **CRUD completeness**: For each primary entity type, verify the golden path test covers ALL of: create, read/view, edit/update, delete. Missing any step = FAIL — write the missing test.
3. **Error state coverage**: Verify at least one E2E test triggers and verifies an error state (e.g., submit form with invalid data, access page without auth, trigger API error). If zero error-state tests exist = FAIL.
4. **Edit flow coverage**: Verify at least one E2E test completes a full edit cycle: navigate to existing entity → click edit → modify fields → save → verify changes persisted. "Create only" coverage is insufficient.
5. **Data persistence verification**: For every form submission test, verify the test checks that data appears correctly AFTER the form submits (on the redirected page, in a list view, or on a detail page). A test that submits a form but doesn't verify the result = SMOKE, not JOURNEY.
6. **Report format**:
   ```
   E2E Coverage Analysis:
   - Features with E2E coverage: X/Y
   - CRUD completeness: create ✓, read ✓, edit ✗, delete ✗
   - Error state tests: N
   - Edit flow tests: N
