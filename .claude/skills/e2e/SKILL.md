...
- **Test features at their full depth, not just their simplest case.** If a feature supports configuration, customisation, or dynamic structure (e.g., custom fields, templates, rules, plugins, dynamic forms, configurable workflows), the tests MUST create a non-trivial configuration and then verify downstream features work with that configuration. Testing only the default or empty configuration is equivalent to not testing. For example: if the system supports custom fields on entities, the E2E test must create an entity type WITH custom fields, then create an entity that POPULATES those fields, then verify the field values appear correctly on the detail page. An entity type with zero fields proves nothing about the field system.
...
7. **Configurable and dynamic features are tested at full depth**: If the application has features that support user-defined configuration — such as custom fields, dynamic forms, configurable schemas, templates, rules, workflows, or plugins — the E2E tests MUST exercise the full configuration-to-usage pipeline:
   - **Configure**: Create a non-trivial configuration through the UI (e.g., add custom fields with different types, create sections, define rules)
   - **Use**: Exercise the feature that consumes that configuration (e.g., create a record that populates custom fields, trigger a configured workflow)
   - **Verify**: Confirm the configured behaviour works correctly downstream (e.g., custom field values appear on detail pages, configured rules fire correctly)

   Testing only with default or empty configuration is equivalent to not testing the configuration system at all. The test must prove that data flows correctly from configuration through to usage.
