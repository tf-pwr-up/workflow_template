...
| **Auth Journeys** | 3 | Login flow works; protected pages enforce access control; logout works |
...
| **Error Handling** | 2 | Form validation errors display correctly; unauthorized access is blocked with visible feedback |
| **Edit Flows** | 1 per editable entity | Load existing data → modify via form → save → verify changes persisted (not just create) |
...
**BLOCKING**: If journey_test_count < 8 OR journey_test_count < smoke_test_count, the E2E suite is INSUFFICIENT. Write more journey tests before proceeding.

**Additional BLOCKING conditions** (any one = INSUFFICIENT):
- No test covers an edit/update flow (load existing → modify → save → verify)
- No test covers form validation errors (submit invalid data → verify error message)
- No test verifies data persistence after creation (submit form → verify data appears on next page/in list)
- Golden path test doesn't cover the full CRUD cycle (create, read, update, AND delete)
